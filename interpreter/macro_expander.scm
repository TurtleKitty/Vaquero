                ...
                (case head
                    ...
                    ((let)
                        (if (check-vaquero-let code)
                           (cons head (vaquero-expand code (noob)))
                           (exit)))
                    ((wall)
                        (if (check-vaquero-wall code)
                           (cons head (vaquero-expand code (local-env)))
                           (exit)))
                    ((quote)
                        ...
                        (if (symbol? head)
                            ...
                            (if (and (pair? head) (eq? (car head) 'send))
                               (let ((pkg-name (cadr head)) (mac-name (caddr head)))
                                    (if (and (symbol? pkg-name) (symbol? mac-name))
                                       (let ((obj (look-it-up pkg-name)))
                                          (if (eq? 'package (vaquero-send obj 'type top-cont expand-err))
                                              (let ((mac (vaquero-send-object obj mac-name top-cont expand-err)))
